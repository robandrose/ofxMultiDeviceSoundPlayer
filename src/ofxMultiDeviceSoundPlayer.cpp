...
void ofxMultiDeviceSoundPlayer::initializeFmodWithTargetDevice(int deviceIndex)
{
	if(!sys_Array_init[deviceIndex]){
		cout << "init Fmod with Target Device"<<deviceIndex<<"\n";
		...
		{
			char name[256];
			//FMOD_System_GetDriverName(sys_Array[deviceIndex], i, name, 256); // Did not work on my mac (?)
			printf("%d : %s\n", i, name);
		}
		*/
		...
		FMOD_System_SetSpeakerMode(sys_Array[deviceIndex],FMOD_SPEAKERMODE_5POINT1); // Put this before INIT to make shure its 5.1
		
		FMOD_System_Init(sys_Array[deviceIndex], 1000, FMOD_INIT_NORMAL, NULL);  //do we want just 32 channels?
...
//------------------------------------------------------------
void ofxMultiDeviceSoundPlayer::setSpeakerMix(float _frontleft, float _frontright, float _center, float _lfe, float _backleft, float _backright){
	...
	if(getIsPlaying() == true){
		FMOD_Channel_SetSpeakerMix(channel, frontleft, frontright, center, lfe, backleft, backright, 0, 0);
	}
	b5pt1=true;
}

void ofxMultiDeviceSoundPlayer::setSpeakerAndVolume(int _speaker, float _volume){
	switch(_speaker){
		case 0:
			setSpeakerMix(_volume, 0,0,0,0,0);
			break;
		case 1:
			setSpeakerMix(0,_volume, 0,0,0,0);
			break;
		case 2:
			setSpeakerMix(0,0,_volume, 0,0,0);
			break;
		case 3:
			setSpeakerMix(0,0,0,_volume, 0,0);
			break;
		case 4:
			setSpeakerMix(0,0,0,0, _volume, 0);
			break;
		case 5:
			setSpeakerMix(0,0,0,0,0, _volume);
			break;
	}
}
